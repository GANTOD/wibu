...
│
└───「 \`\`\`${NamaBot}\`\`\` 」`
sendButLocation(from, `${menu}`, "*_© Gant_*", {jpegThumbnail:ofrply,name:""}, [{buttonId:`command`,buttonText:{displayText:'BUTTON MENU'},type:1},{buttonId:`owner`,buttonText:{displayText:'DEVELOPER'},type:1},{buttonId:`script`,buttonText:{displayText:'SOURCE CODE'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
break
case 'allmenu': 
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
stod = `${sender}`
       stst = await wibu.getStatus(`${sender.split('@')[0]}@c.us`)
       nama = '🏴‍☠️ Gant 🏴‍☠️'
       ownerJid = "6282192943105@s.whatsapp.net"
				stst = stst.status == 401 ? '' : stst.status
			num = await fetchJson(`https://api.telnyx.com/anonymous/v2/number_lookup/${senderNumber}`, {method: 'get'})
menunya = `╭─❒ 「 Bot Info 」 ❒
🏴‍☠️ _Creator : *Gant* (CUMAN RECODE)
...
break
		...
		case 'sourcecode':
		anu =`*wibu SEWABOT*
		10K PERMANEN BWANG:) RUN RDP 24JAM
		MINAT PC OWNER SAJA`
 buttons = [
...
*│*⬡ *PULSA     :* ${ovo}
*│*⬡ *DANA    : ${dana}*
*│*⬡ *GOPAY    * https://l.top4top.io/p_21282fcg00.jpg
*└───────────────────*
Scan QR gopay diatas `
